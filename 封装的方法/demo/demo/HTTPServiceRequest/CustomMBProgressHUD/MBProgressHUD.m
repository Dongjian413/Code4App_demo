...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		[indicator removeFromSuperview];
		self.indicator = nil;
    }else if (mode == MBProgressHUDMyCustomAnimation){
        [indicator removeFromSuperview];
        self.indicator = MB_AUTORELEASE([[UIActivityIndicatorView alloc]
                                         initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge]);
        self.indicator = MB_AUTORELEASE([[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 140, 80)]);
        self.indicator.contentMode = UIViewContentModeScaleAspectFit;
        //          [(UIActivityIndicatorView *)indicator startAnimating];
        [self addSubview:indicator];
        
#pragma mark---------我的HUD的自定义动画
        NSString *resourcePath=[NSBundle mainBundle].resourcePath;
        NSMutableArray *AnimationArray = [NSMutableArray array];
        for (int i=1; i<7; i++) {
            //图像路径
            NSString *path = [resourcePath stringByAppendingPathComponent:[NSString stringWithFormat:@"Animation_HUD%d.png",i]];
            //创建图片
            UIImage *image = [UIImage imageWithContentsOfFile:path];
            [AnimationArray addObject:image];
        }
        //设置动画数组
        [(UIImageView *)self.indicator setAnimationImages:AnimationArray];
        //设置动画播放次数
        [(UIImageView *)self.indicator setAnimationRepeatCount:MAXFLOAT];
        //设置动画播放时间
        [(UIImageView *)self.indicator setAnimationDuration:0.8];
        //开始动画
